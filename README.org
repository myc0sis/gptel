...
| NanoGPT              | [[https://nano-gpt.com/][API key]]                    |
...
#+begin_src emacs-lisp
;; NanoGPT offers an OpenAI compatible API
(gptel-make-nanogpt "NanoGPT"               ;Any name you want
  :key "your-api-key"                       ;can be a function that returns the key
  :models '("moonshotai/kimi-k2.5"))
#+end_src

If you want to drive configuration via environment variables (for example,
=NANOGPT_API_URL=, =NANOGPT_MODEL=, =NANOGPT_CONFIRM=, =NANOGPT_TIMEOUT=):

#+begin_src emacs-lisp
(let* ((api-url (getenv "NANOGPT_API_URL"))
       (model (or (getenv "NANOGPT_MODEL") "moonshotai/kimi-k2.5"))
       (confirm (string= (or (getenv "NANOGPT_CONFIRM") "") "true"))
       (timeout (getenv "NANOGPT_TIMEOUT")))
  (gptel-make-nanogpt "NanoGPT"
    :key (lambda () (getenv "NANOGPT_API_KEY"))
    :models (list model)
    :request-params (and confirm '(:confirm t))
    :curl-args (and timeout (list "--max-time" timeout))
    :url api-url))
#+end_src
